package com.gc.util
{
  ...
  public class ObjectUtilEx
  {
    ...
    public function ObjectUtilEx(caller:Function=null)
    {
      if (caller != singleton)
        throw new Error("ExtObjectUtil is a non-instance class!!!");
    }

    /**
     * Returns true or false if the object contains the provided string
     */
    public static function contains(item:Object, string:String, partial:Boolean=false, caseSensitive:Boolean=false):Boolean
    {
      var strTemp:String;
      if (!caseSensitive)
      ...
      for each (var key:String in item)
      {
        ...
        if (partial && strTemp.search(string) >= 0)
          return true;
      }
      return false;
    }

    public static function toURLs(object:Object):URLVariables
    {
      var urlVars:URLVariables=new URLVariables;
      for each (var key:String in object)
      {
        urlVars[key]=object[key].toString();
      }
      return urlVars;
    }

    public static function cast(genericObj:Object, typedObj:Object):Object
    {
      for each (var key:String in genericObj)
      {
        if (key != "mx_internal_uid")
        {
          if (typedObj.hasOwnProperty(key))
          {
            try
            {
              //trace( typedObj[key] is Date );
              if (typedObj[key] is Date)
              {
                typedObj[key]=new Date(genericObj[key]);
              }
              else
              {
                typedObj[key]=genericObj[key];
              }
            }
            catch (err:Error)
            {
              trace("ERROR! GenericToTypedObject :" + toString(err));
            }
          }
        }
      }
